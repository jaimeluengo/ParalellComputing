...
void omp_sort_block(int n_threads, int m, int n, int *mat[n]){
    int maxRow = n<=m ? n : m;
    for(int i=0; i<maxRow; i++){
        int colmax_total = i, rowmax_total = i, max_total = mat[i][i];
		...
		//Just swap if the max row/column is not the current one
       if(i!= rowmax_total) swap_rows_open_mp(n_threads, rowmax_total, i, m, n, mat); 
       if(i!= colmax_total) swap_cols_open_mp(n_threads, colmax_total, i, n, mat); 
    }
}
